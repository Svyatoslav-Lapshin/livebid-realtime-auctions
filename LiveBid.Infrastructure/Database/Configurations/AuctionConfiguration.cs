...
namespace LiveBid.Infrastructure.Database.Configurations
{
    public sealed class AuctionConfiguration : IEntityTypeConfiguration<Auction>
    {
        public void Configure(EntityTypeBuilder<Auction> builder)
        {
            ...
            builder.Property(auction => auction.StartPrice)
                .HasColumnName("start_price")
                .HasPrecision(18, 2)
                .IsRequired();

            builder.Property(auction => auction.CurrentPrice)
                .HasColumnName("current_price")
                .HasPrecision(18, 2)
                .IsRequired();

            builder.Property(auction => auction.StartTime).
                HasColumnName("start_time")
                .IsRequired();

            builder.Property(auction => auction.EndTime).HasColumnName("end_time").IsRequired();

            builder.Property(auction => auction.Status).HasColumnName("status")
                .HasConversion(status => status.ToString()
                .ToLowerInvariant(), value => Enum.Parse<AuctionStatus>(value, ignoreCase: true))
                .HasMaxLength(30)
                .IsRequired();
